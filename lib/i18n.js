(function(window) {
  var I18n, assert, findTemplate, isBinding, lookupKey, pluralForm,
  ...
  EmHandlebars.registerHelper('t', function(key, options) {
    ...
    if (key.charAt(0) === '.') {
      var name = options.data.view.renderedName;
      if(!name){
        var parentView = options.data.view._parentView;
        // Iterate up view stack to find context
        while(parentView){
          name = parentView.renderedName;
          parentView = (!!name) ? null : parentView._parentView;
        }
      }
      key = name + key;
    }
    ...
    Ember.keys(attrs).forEach(function(property) {
      ...
      var currentValue = EmHandlebars.get(context, bindPath, options);
      ...
      var observer = function() {
        ...
        attrs[propertyName] = EmHandlebars.get(context, bindPath, options);
  ...
  var attrHelperFunction = function(options) {
    ...
    Ember.keys(attrs).forEach(function(attr) {
      ...
      normalized = EmHandlebars.normalizePath(ctx, "translations."+path, options.data);

      var value = I18n.t(path);

      var observer;

      observer = function observer() {
        var result = EmHandlebars.get(ctx, normalized.path, options);

        var elem = view.$("[data-" + dataId + "='" + dataId + "']");

        if (!elem || elem.length === 0) {
          view.removeObserver(normalized.root, normalized.path, observer);
          return;
        }
      ...
      ret.push(attr + '="' + EmHandlebars.Utils.escapeExpression(value) + '"');

    });

    ret.push('data-' + dataId + '="' + dataId + '"');
